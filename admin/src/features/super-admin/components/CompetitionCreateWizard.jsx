...
// Non-city geographic scopes. When one of these is selected, the competition is
// not tied to a single city (city_id is left null) and `geographic_scope` records it.
const SCOPE_OPTIONS = [
  { value: 'usa', label: 'USA — Nationwide', name: 'USA' },
  { value: 'worldwide', label: 'Worldwide', name: 'Worldwide' },
];
const SCOPE_NAME = SCOPE_OPTIONS.reduce((acc, s) => { acc[s.value] = s.name; return acc; }, {});
...
const INITIAL_FORM = {
  organization_id: '',
  geographic_scope: 'city', // 'city' | 'usa' | 'worldwide'
  city_id: '',
  category_id: '',
  demographic_id: '',
  name: '',
...
export default function CompetitionCreateWizard({
  ...
  // Location dropdown: non-city scopes (USA / Worldwide) first, then cities.
  const locationOptions = useMemo(() => [
    ...SCOPE_OPTIONS.map(s => ({ value: `scope:${s.value}`, label: s.label })),
    ...cities.map(c => ({ value: c.id, label: `${c.name}, ${c.state}` })),
  ], [cities]);

  // The select's current value: a scope sentinel, or the selected city id.
  const locationValue = formData.geographic_scope === 'city'
    ? formData.city_id
    : `scope:${formData.geographic_scope}`;

  const handleLocationChange = (value) => {
    if (value.startsWith('scope:')) {
      setFormData(prev => ({ ...prev, geographic_scope: value.slice('scope:'.length), city_id: '' }));
    } else {
      setFormData(prev => ({ ...prev, geographic_scope: 'city', city_id: value }));
    }
  };

  const locationSelected = formData.geographic_scope !== 'city' || !!formData.city_id;
  ...
  const autoGeneratedName = useMemo(() => {
    ...
    const locationName = formData.geographic_scope === 'city'
      ...
      : SCOPE_NAME[formData.geographic_scope];
  ...
  }, [formData.organization_id, formData.geographic_scope, formData.city_id, formData.demographic_id, organizations, cities, demographics]);
    ...
    >
      ...
      {currentStep === 2 && (
        <FormSection title="Location & Season" divider={false}>
          <FormGrid>
            <FormField label="Location" required description="A city, or nationwide / worldwide">
              <SelectInput
                value={locationValue}
        ...
        <FormSection title="Competition Details" divider={false}>
          <FormGrid>
            ...
            {formData.geographic_scope === 'city' ? (
              ...
              <FormField label="Eligibility" description="No location restriction for this scope">
                <div style={styles.scopeNote}>
                  {formData.geographic_scope === 'usa'
...
function ReviewStep({ formData, organizations, cities, categories, demographics, hosts, getHostName }) {
  ...
  const locationName = isScoped ? SCOPE_NAME[formData.geographic_scope] : selectedCity?.name;
  ...
  const rows = [
    ...
    { label: 'Minimum Prize', value: `$${formData.minimum_prize.toLocaleString()}` },
    {
      label: 'Eligibility',
      value: isScoped
        ? (formData.geographic_scope === 'usa' ? 'Nationwide (USA)' : 'Worldwide')
          ...
          <p style={{ color: colors.gold.primary, fontSize: typography.fontSize.sm, margin: `${spacing[1]} 0 0` }}>
            {getCompetitionUrl(
              ...
              generateCompetitionSlug({
                ...
                citySlug: selectedCity?.slug || formData.geographic_scope,
