...
/**
 * send-round-end-emails — Emails every contestant in a finalized voting round
 * their result.
 *
 * Invoked by the round-end-emails cron (every 5 min) ~3 hours after a round is
 * finalized. Reads voting_rounds.finalized_snapshot and, for each entry, calls
 * send-onesignal-email with type `round_advanced` (advanced/winner) or
 * `round_eliminated` (eliminated/runner_up).
 *
 * Idempotency: voting_rounds.round_end_emails_sent_at is stamped once at least
 * one email succeeds. A total failure leaves it unstamped so the cron retries.
 *
 * Body: { round_id: string, verify_only?: boolean }
 *   verify_only — build payloads and count recipients without sending or
 *   stamping the round. Useful for a dry run before a transition.
 */
