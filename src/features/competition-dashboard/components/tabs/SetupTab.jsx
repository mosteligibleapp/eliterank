...
export default function SetupTab({
  ...
  return (
    <div>
      ...
      {/* Prizes Section (Rotating Rewards Carousel) */}
      <Panel
        title={`Prizes (${prizes.length})`}
        icon={Gift}
        action={<Button size="sm" icon={Plus} onClick={() => onOpenPrizeModal(null)}>Add Prize</Button>}
        collapsible
        defaultCollapsed
      >
        <div style={{ padding: isMobile ? spacing.md : spacing.xl }}>
          {prizes.length === 0 ? (
            <div style={{ textAlign: 'center', padding: spacing.xl, color: colors.text.secondary }}>
              ...
              <p style={{ fontSize: typography.fontSize.sm, color: colors.text.muted, marginTop: spacing.sm }}>
                Add prizes to display in the rotating rewards carousel on your competition page.
              </p>
            </div>
          ) : (
            <div style={{ display: 'grid', gap: spacing.md }}>
